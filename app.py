"""
Max Gym GHL — Flask Static HTML Server
=======================================
Sirve HTMLs estáticos via send_from_directory + nav inyectado.
Cada request GET /<route> sirve el HTML correspondiente del directorio templates/.
...
# ─── Route → template filename mapping ──────────────────────────────────────
ROUTES = {
    '/dashboard':          'Sprint_Dashboard_MAX_GYM.html',
    '/ghl-workflows':    'workflows-dashboard.html',
    '/strategy-brief':   '01_Strategy_Brief.html',
...
# Ordered nav items (route, label, icon)
NAV = [
...
def nav_html(current_path: str) -> str:
    ...
    items = ''
    for route, label, desc in NAV:
        active = 'active' if current_path == route else ''
        items += f'<li><a href="{route}" class="{active}" title="{desc}">{label}</a></li>\n'
...
def inject_nav(content: bytes, current_path: str) -> bytes:
    """Inject nav bar into an HTML file after <body> or <body class=...>."""
    ...
    if body_tag in content:
        ...
        return content[:end_idx].decode() + nav + content[end_idx:].decode()
    else:
        return nav + content.decode()
